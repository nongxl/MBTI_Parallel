...
// 6 维多边形内点判定算法 (Ray-Casting Point-in-Polygon Engine)
static bool pointIn6Gon(int px, int py, const int vx[6], const int vy[6]) {
    bool c = false;
    for (int i = 0, j = 5; i < 6; j = i++) {
        if (((vy[i] > py) != (vy[j] > py)) &&
            (px < (vx[j] - vx[i]) * (py - vy[i]) / static_cast<float>(vy[j] - vy[i]) + vx[i])) {
            c = !c;
        }
    }
    return c;
...
// 【Phase 6A】双雷达叠加多边形绘制 (双色物理半透明 + 重叠加深交集引擎)
void drawDualRadarChart(M5Canvas& canvas, int centerX, int centerY, int radius, const RadarData& userData, const RadarData& mbtiData, const char* mbtiName, bool drawLabels, bool isCN) {
    // 1. 计算 YOU 6 维顶点
    float userValues[6] = { userData.novelty, userData.risk, userData.planning, userData.practicality, userData.logic, userData.social };
    int ux[6], uy[6];
    for (int i = 0; i < 6; ++i) {
        float val = std::min(100.0f, std::max(0.0f, userValues[i]));
        float r = radius * (val / 100.0f);
        float angle = -M_PI / 2.0f + i * (M_PI / 3.0f);
        ux[i] = centerX + static_cast<int>(r * cosf(angle));
        uy[i] = centerY + static_cast<int>(r * sinf(angle));
    }

    // 2. 计算 MBTI Archetype 6 维顶点
    float mbtiValues[6] = { mbtiData.novelty, mbtiData.risk, mbtiData.planning, mbtiData.practicality, mbtiData.logic, mbtiData.social };
    int mx[6], my[6];
    for (int i = 0; i < 6; ++i) {
    ...
    // 3. 【双色半透明 + 重叠交集物理加深引擎】
    int minX = std::max(0, centerX - radius - 2);
    int maxX = std::min(239, centerX + radius + 2);
    int minY = std::max(0, centerY - radius - 2);
    int maxY = std::min(134, centerY + radius + 2);

    for (int y = minY; y <= maxY; ++y) {
        for (int x = minX; x <= maxX; ++x) {
            bool inUser = pointIn6Gon(x, y, ux, uy);
            ...
            if (inUser && inMbti) {
                ...
                if (bg == 0) canvas.drawPixel(x, y, 0x3013);
                    ...
                    uint8_t rB = (bg >> 11) & 0x1F, gB = (bg >> 5) & 0x3F, bB = bg & 0x1F;
                    canvas.drawPixel(x, y, (((rB + 4) >> 1) << 11) | ((gB >> 1) << 5) | ((bB + 16) >> 1));
                }
            } else if (inUser) {
                // 【YOU 独享区】: 淡发光水青色 Alpha
                uint16_t bg = canvas.readPixel(x, y);
                if (bg == 0) canvas.drawPixel(x, y, 0x016A);
            } else if (inMbti) {
                // 【MBTI 独享区】: 淡发光霓虹品紫 Alpha (Light Magenta 0x380D)
                uint16_t bg = canvas.readPixel(x, y);
                if (bg == 0) canvas.drawPixel(x, y, 0x380D);
            }
        }
    }
    ...
    // 5. 【图层 4: 前景 YOU 亮绿实线 (GREEN) + 实体白点】
    for (int i = 0; i < 6; ++i) {
        int next = (i + 1) % 6;
        canvas.drawLine(ux[i], uy[i], ux[next], uy[next], GREEN);
        canvas.drawLine(ux[i] + 1, uy[i], ux[next] + 1, uy[next], GREEN);
    }
    for (int i = 0; i < 6; ++i) {
        canvas.fillCircle(ux[i], uy[i], 2, WHITE);
    }

    // 6. 【图层 5: 前景 MBTI 霓虹紫实线 (MAGENTA) + 实体紫点】
